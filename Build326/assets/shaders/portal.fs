...
{           
    ...
    float portalDistance = 2.0 * NEARPLANE * FARPLANE / (FARPLANE + NEARPLANE - (2.0 * depth - 1.0) * (FARPLANE - NEARPLANE));
    float portalDepth = floorDistance - portalDistance;
    float maxVisableDepth = 2.5;
    float portalBorderDepth = 0.3;

    if (portalDistance >= floorDistance)
        discard;
    ...
    FragColor = lerp(
                    ...
                    clamp(portalDepth/(portalBorderDepth), 0.0, 1.0)
                );
}
